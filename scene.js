...
$(‘loader-pct’).textContent = String(Math.round(pct)).padStart(3,‘0’) + ’ %’;
}
},
err => {
clearTimeout(gltfTimeout);
...
Beats (scroll % ranges):
1  0-8    Void / Monogram
2  8-22   Sacred Tree (bg-hero)
3  22-38  Dialogue
4  38-50  Ascent / Clouds (bg-clouds)
5  50-62  Traverse & Thunder
6  62-72  Impact & Shatter
7  72-85  Skill Hub (bg-paper)
8  85-96  Moonlit Hill (bg-hilltop)
9  96-100 Dawn (bg-dawn)
*/
...
// Weights per image — cross-fade with overlap
...
// Parallax: scale + translate using scroll progress + mouse
// Each layer has its own Ken-Burns-style gentle motion so bg feels alive
...
// rest top-right above Skill Hub
...
// GOD RAYS during cloud + dawn beats
...
// EMBERS during beats 4-5 (clouds)
...
// DANDELION SEEDS during beats 8-9 (hilltop + dawn)
...
// Hero monogram — visible in beat 1 only
toggle($(‘hero-monogram’), ‘-on’, b === 1);

// Captions — tied to beat
...
host.innerHTML = WORKS.map(w => `<div class="sh-row" data-cursor="hover"> <div class="sh-num">N° ${w.no}</div> <div class="sh-title-row">${w.title}</div> <div class="sh-role">${w.role}</div> <div class="sh-year">${w.year}</div> <div class="sh-desc">${w.desc}</div> </div>`).join(’’);
}
buildSkillHub();
...
if(e.target.closest(’[data-cursor=“hover”], button, .sh-row, .ch-item, .chapter-pill’)){
...
if(ldP > 24 && ldStage === 1){ ldStage = 2; ldText.textContent = STAGES[2]; }
...
// preload assets then mark ready
preloadImages([
‘assets/bg-hero.png’,

/* =================================================================
sumanth.D (c) - SCENE ENGINE v6 FINAL

- Breathing zones between beats (no congestion)
- Sumi ink bleed replaces shatter
- Jelly glass mouse tracking on titles
- Interactive scrollable emaki vault on Luffy click
- Sword: scroll-driven only, slight parallax, never clips
- Hero monogram fits viewport
- ASCII-only code paths (no smart quotes)
  ================================================================= */
...
/* ============== BEAT LAYOUT WITH BREATHING ZONES ==============

00-07%   Void (monogram)
07-12%   breathe
12-22%   Sacred Tree reveal
22-28%   breathe / dialogue 1-2
28-38%   Dialogue title + dialogue 3-4
38-44%   breathe / dialogue 5 (answer)
44-54%   Ascent (clouds appear)
54-62%   Thunder traverse
62-68%   Sumi bleed (replaces shatter)
68-72%   breathe (bleed settles, paper emerges)
72-78%   Skill Hub INTRO
78-82%   breathe
82-86%   Skill Hub LEDGER (7 works)
86-90%   breathe
90-94%   Skill Hub FEATURE (Fudo Myoo)
94-98%   Moonlit hill
98-100%  Dawn + Luffy
...
/* ============== HIDDEN VAULT (revealed on awakening) ============== */
var VAULT = [
...
/* Separates animation transform from centering transform */
[‘bt-sacred’,‘bt-dialogue’,‘bt-ascent’,‘bt-self’].forEach(function(id){
var el = $(id);
if(!el) return;
var inner = document.createElement(‘div’);
inner.className = ‘bt-inner’;
// initial HTML is empty (populated on scramble); wrap whatever’s there
...
console.warn(‘GLTF timeout - standin’);
var s = createStandin();
...
function updateBg(){
var p = S.progress;
// With breathing zones - longer hold times, smoother crossfades
...
function setOp(el, w){
if(!el) return;
el.style.opacity = w > 0.01 ? Math.min(1, w) : 0;
}
...
/* ============== KATANA CHOREOGRAPHY (scroll only, no cursor following) ============== */
var lastSmooth = 0;
...
// DIALOGUE - gentle float, held steady mid-frame
...
// SKILL HUB - sword rests small top-right, never comes close
...
// SLIGHT cursor parallax only (not cursor following)
kx += S.nx*0.015;
ky += S.ny*0.015;

// CLAMP Z - never let sword come too close (prevents self-clip)
if(kz < 1.0) kz = 1.0;

KAT.position.set(kx, ky, kz);
KAT.rotation.set(rX, rY, rZ);

// Light color per beat
if(p < 0.38){
...
/* ============== SUMI BLEED (replaces shatter) ============== */
function updateSumiBleed(){
var p = S.progress;
var el = $(‘sumi-bleed’);
if(!el) return;
// Active during 0.62 - 0.72, reverses on scroll back
...
el.style.setProperty(’–bleed’, bleed * 0.6); // 60% max - leaves center visible
}
...
// SKILL HUB - 3 stages with phase gates and breathing
var inHub = p >= 0.72 && p < 0.94;
toggle($(‘skillhub’), ‘-on’, inHub);
$(‘skillhub’).setAttribute(‘aria-hidden’, inHub ? ‘false’ : ‘true’);
...
+  ‘<div class="vc-body">’
+  ’<div class="vc-no">N ’ + v.no + ’ · ’ + v.tag + ‘</div>’
...
// scroll ink-system to top on open
...
// Falling ink droplets/petals/stars on Z
